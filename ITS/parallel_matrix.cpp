...
int main()
{
	...
	{
		#pragma omp parallel for
        // iteration of the outer loop will be executed in parallel by multiple threads.
		...
		for(int j=0;j<col2;j++)
		{
            ...
            // The reduction clause ensures that each thread has its own private sum, and at the 
            // end of the loop, all private sum values are combined using the + operator to obtain the final sum.
